...
- **Satellite imagery overlays** — daily VIIRS true-color and nighttime-lights layers (NASA GIBS, keyless) plus a sub-meter baseline layer (Esri World Imagery — undated archive mosaic, explicitly labeled as such) for site identification; grid blackouts after infrastructure strikes are directly visible in the night-lights band
